...
var SysEnum = {
    ...
    Load: function () {
        ...
        $.ajax({
            url: "/sw/ScriptServices/AdminService.asmx/GetJsonForSysEnum",
            type: "post",
            data: "{}",
...
//                    }
    ...
    Add: function () {
        SysEnum.Url = "/sw/ScriptServices/AdminService.asmx/SaveSysEnum";
        var t = $("#treeCt");
        ...
        if (node) {
            $.messager.confirm('温馨提醒', '确定要删除吗?', function (r) {
                if (r) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/sw/ScriptServices/AdminService.asmx/DelSysEnum",
                        contentType: "application/json; charset=utf-8",
                        data: "{id:'" + node.id + "'}",
                        beforeSend: function () {
                            $("#dlgWaiting").dialog('open');
    ...
    Save: function () {
        ...
        $.ajax({
            url: SysEnum.Url,
            type: "post",
            data: '{model:{Id:"' + $("#hId").val() + '",EnumName:"' + $("#txtName").val() + '",EnumCode:"' + $("#txtCode").val() + '",EnumValue:"' + $("#txtValue").val() + '",ParentId:"' + $("#hParentId").val() + '",Sort:' + sSort + ',Remark:"' + $("#txtRemark").val() + '"}}',
            contentType: "application/json; charset=utf-8",
            beforeSend: function () {
                $("#dlgWaiting").dialog('open');
            },
