...
var ListUser = {
    ...
    OnIsLockedOut: function (h) {
        try {
            ...
            $.messager.confirm('温馨提醒', '确定要执行此操作吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/sw/ScriptServices/AdminService.asmx/SaveIsLockedOut",
                        type: "post",
    ...
    OnIsApproved: function (h) {
        try {
            ...
            $.messager.confirm('温馨提醒', '确定要执行此操作吗？', function (r) {
                if (r) {
                    $.ajax({
                        url: "/sw/ScriptServices/AdminService.asmx/SaveIsApproved",
                        type: "post",
                        data: "{userName:'" + currObj.attr("code") + "'}",
                        contentType: "application/json; charset=utf-8",
                        beforeSend: function () {
        ...
        try {
            ...
            $.messager.confirm('温馨提醒', '确定要删除吗？', function (r) {
                if (r) {
                    ...
                    $.ajax({
                        url: "/sw/ScriptServices/AdminService.asmx/DelUser",
                        type: "post",
                        contentType: "application/json; charset=utf-8",
    ...
    CbIsInRole: function (h) {
        try {
            ...
            $.ajax({
                url: "/sw/ScriptServices/AdminService.asmx/SaveUserInRole",
                type: "post",
                contentType: "application/json; charset=utf-8",
    ...
    OnRoleLoadSuccess: function (data) {
        try {
            ...
            if (rows && rows.length > 0) {
                $.ajax({
                    url: "/sw/ScriptServices/AdminService.asmx/GetUserInRole",
                    type: "post",
                    contentType: "application/json; charset=utf-8",
                    data: '{userName:"' + userName + '"}',
                    beforeSend: function () {
    ...
    AddUserHouseOwner: function () {
        ...
        window.location = "/a/ata.html?userName=" + cbl[0].f1 + "";
    },
    EditUserBase: function () {
        var rows = $('#bindT').datagrid("getSelections");
        if (!rows || rows.length != 1) {
            $.messager.alert('错误提醒', '请选择一行且仅一行数据进行操作', 'error');
            return false;
        }
        window.location = "http://hnztc.tygaweb.com:18881/a/tub.html?userName="+rows[0].f1+"";
    }
}