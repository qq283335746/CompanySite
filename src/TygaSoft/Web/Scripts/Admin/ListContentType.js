...
var ListContentType = {
    ...
    Load: function () {
        ...
        $.ajax({
            url: "/sw/ScriptServices/AdminService.asmx/GetJsonForContentType",
            type: "post",
            data: "{}",
    ...
    Add: function () {
        this.Url = "/sw/ScriptServices/AdminService.asmx/SaveContentType";
        var t = $("#treeCt");
        ...
        if (node) {
            $.messager.confirm('温馨提醒', '确定要删除吗?', function (r) {
                if (r) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "/sw/ScriptServices/AdminService.asmx/DelContentType",
                        contentType: "application/json; charset=utf-8",
                        data: "{id:'" + node.id + "'}",
                        beforeSend: function () {
                            $("#dlgWaiting").dialog('open');
    ...
    Save: function () {
        ...
        if (sSort == "") sSort = 0;
        var sPictureId = $.trim($("#imgContentPicture").parent().find("input[type=hidden]").val());
        if (sPictureId == "") sPictureId = "00000000-0000-0000-0000-000000000000";

        $.ajax({
            url: ListContentType.Url,
            type: "post",
            data: '{model:{Id:"' + $("#hId").val() + '",TypeName:"' + $("#txtName").val() + '",TypeCode:"' + $("#txtCode").val() + '",TypeValue:"' + $("#txtValue").val() + '",ParentId:"' + $("#hParentId").val() + '",PictureId:"' + sPictureId + '",Sort:' + sSort + '}}',
            contentType: "application/json; charset=utf-8",
            beforeSend: function () {
                $("#dlgWaiting").dialog('open');
            },
    ...
    CreateShareMenu: function () {
        $.messager.confirm('温馨提醒', '确定要生成吗?', function (r) {
            if (r) {
                $.ajax({
                    url: "/sw/ScriptServices/AdminService.asmx/CreateShareMenu",
                    type: "post",
                    data: '{typeCode:"ShareMenu"}',
                    contentType: "application/json; charset=utf-8",
                    beforeSend: function () {
